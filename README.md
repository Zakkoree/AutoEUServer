...
| `TG_BOT_TOKEN` | Telegram Bot 的 Token                             | 是 |
| `TG_USER_ID` | Telegram 接收消息的 Chat ID                           | 是 |
| `YESCAPTCHA_KEY` | YesCaptCha API 密钥 (用于处理复杂人机验证)                   | 是 |
| `MAILPARSER_DOWNLOAD_URL_ID` | Mailparser & Parseur 或类似邮件解析服务的下载地址 (用于自动获取 PIN) | 是 |

### 3. 启动运行
配置完成后，Actions 会根据设定的 Cron 表达式自动运行。你也可以在 `Actions` 页面手动触发一次运行以测试配置是否生效。
